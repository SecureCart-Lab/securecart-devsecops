...
for ((attempt=1; attempt<=MAX_ATTEMPTS; attempt++)); do
    ...
    if (( attempt == MAX_ATTEMPTS )); then
        printf 'ERROR: SecureCart did not report status UP after %s attempts.\n' "$MAX_ATTEMPTS" >&2
        exit 1
    fi
...
printf '\nHealth:\n'
curl -fsS "$BASE_URL/actuator/health" | jq -e '.status == "UP"'

printf '\nProducts:\n'
products_json="$(curl -fsS "$BASE_URL/api/products")"
jq . <<<"$products_json"
jq -e 'type == "array" and length > 0' >/dev/null <<<"$products_json"
